...
func (pc *participantController) GetParticipant(c *gin.Context) {
	...
	participants, restErr := pc.participantService.GetParticipant(getParticipartRequest.EditionId, getParticipartRequest.CursorCreatedAt, getParticipartRequest.CursorUserTime, getParticipartRequest.WorstTime)
	if restErr != nil {
		c.JSON(restErr.Code, restErr)
		return
	}
