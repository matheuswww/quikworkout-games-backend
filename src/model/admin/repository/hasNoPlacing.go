...
func (ar *adminRepository) HasPlacing(edition_id, category, sex string) (bool, *rest_err.RestErr) {
	...
	count = 0
	query = "SELECT COUNT(*) FROM participant WHERE placing IS NOT NULL AND edition_id = ? AND category = ? AND sex = ? LIMIT 1"
	err = ar.mysql.QueryRowContext(ctx, query, edition_id, category, sex).Scan(&count)
	if err != nil {
