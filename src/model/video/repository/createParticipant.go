...
import (
	...
	participant_domain "github.com/matheuswww/quikworkout-games-backend/src/model/video"
	"go.uber.org/zap"
)
...
func (cr *participantRepository) CreateParticipant(participantDomain participant_domain.ParticipantDomainInterface, instagram string) *rest_err.RestErr {
	...
	query := "INSERT INTO participant (video_id, user_id, edition_id, active) VALUES (?, ?, ?, ?)"
	_,err = tx.ExecContext(ctx, query, participantDomain.GetVideoID(), participantDomain.GetUserID(), participantDomain.GetEditionID(), participantDomain.GetActive())
	if err != nil {
